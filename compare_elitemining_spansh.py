...
USER_AGENT = "EliteMining-Spansh-Compare/3.0"
...
def fetch_spansh(session, systems, delay=1.6, page_size=500):
    """
    IMPORTANT V3:
    Query ALL bodies for the requested systems, not only bodies filtered by rings.

    We track three independent layers:
      1) systems_seen  -> any body from that system was returned
      2) rings_seen    -> that exact ring exists in Spansh body data
      3) hotspots      -> that exact material signal exists on that ring

    This prevents a known system with no hotspot signals from being
    incorrectly labelled MISSING_SYSTEM.
    """
    hotspots = {}
    systems_seen = set()
    rings_seen = set()
...
def main():
    ...
    for k in missing:
        ...
        if system_key not in spansh_systems_seen:
